...
async def debug_api(alert_type="disk_full", api_url="http://localhost:8000"):
    """
    Testa o endpoint de debug da API Dorothy.
    ...
    endpoint = f"{api_url}/api/v1/zabbix/alert/debug"
    
    print(f"\n=== TESTE DE DEBUG PARA ALERTA: {alert_type} ===\n")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                endpoint,
                json=payload,
                timeout=60.0  # Tempo maior para debug
            )
            ...
            if response.status_code == 200:
                ...
                print(f"Modelo usado: {details.get('ollama_model')}")
                print(f"Tempo de processamento LLM: {details.get('ollama_time_seconds')}s")
                print(f"Tempo total API: {details.get('total_time_seconds')}s")
                print("-" * 70)
                
                # Exibe a análise
                analysis = result.get("analysis", {})
                print(f"Ação recomendada: {analysis.get('action')}")
                print(f"Confiança: {analysis.get('confidence')}")
                print(f"Motivo: {analysis.get('reason')}")
                
                # Exibe parâmetros
                if "job_parameters" in analysis:
                    print("\nParâmetros:")
                    for key, value in analysis.get("job_parameters", {}).items():
                        print(f"  - {key}: {value}")
                
                # Exibe a resposta completa em formato JSON para referência
                print("\nResposta completa:")
...
if __name__ == "__main__":
    ...
    asyncio.run(debug_api(alert_type, api_url))